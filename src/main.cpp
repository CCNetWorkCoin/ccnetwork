...
#if defined(NDEBUG)
# error "CcNetwork cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x272407b967eaf5aef5b754aa7956eb9af8277c37f331ccf1a16570ab385c0c7f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CcNetwork: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CcNetwork Signed Message:\n";
...
{
    // CcNetwork: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // CcNetwork: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // CcNetwork: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // CcNetwork: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CcNetwork: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CcNetwork: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x586502ef4cfc7fd023e60be0da836c6246b6848a5359cb6dbf21a2b110d23140");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "On 11/16/2019 CCNetWork has been born - 854";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041ab402faa4267852e822f6b96717295f403afdcd1149b40239b1510cdcb0f408beaf3ce57f7afe98d9bebd0a18f26258bf72f596b9793c57b12a0b480a800161") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1573926820;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086071810;

        if (fTestNet)
        {
            block.nTime    = 1573926769;
            block.nNonce   = 385734373;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe85f0b7e42204f2fe567b7259b71de7415051a251f0754956c3ad1f4715dc08b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xc3, 0xbd, 0xd3 }; // CcNetwork: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CcNetworkMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CcNetworkMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CcNetworkMiner(CWallet *pwallet)
{
    printf("CcNetworkMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ccnetwork-miner");
    ...
    try { loop {
        ...
        printf("Running CcNetworkMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CcNetworkMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CcNetworkMiner, pwallet));
}
